...
-- @[extern "append_simp"]
-- def simp_lemmas.append_simp : simp_lemmas → List name → tactic simp_lemmas
-- | sls []      := return sls
-- | sls (l::ls) := do
...
/-
  at_target is only used once, perhaps not worth the effort of re-implement it

  the main logic seems simple:
  - first run a tac function on the current target to get a pair, i.e., a new target and some proof (for old target??)
  -
...
-- def atTarget (tac : Expr → MetaM (Expr × Expr)) : TacticM Unit := do
--   let tgt ← getMainTarget
--   let (newTgt, pf) ← tac tgt -- liftMetaTactic λ g => tac g.target
--   -- let n ← mkFreshId
--   -- let n ← mkFreshExprMVar newTgt
--   let n ← mkFreshMVarId
--   -- assert n newTgt
--   Lean.MVarId.assertHypotheses n #[newTgt]
--   swap
--   let ht ← getLocalDecl n.mvarId!
--   exact (← mkEqMpr pf ht.toExpr)

-- ChatGPT second try
-- def atTarget (tac : Expr → TacticM (Expr × Expr)) : TacticM Unit := do
--   let tgt ← getMainTarget
--   let (newTgt, pf) ← tac tgt
--   let n ← mkFreshExprMVar newTgt

--   let mvarId ← getMainGoal
--   let mvarId' ← assert mvarId n.mvarId! newTgt
--   swapGoal mvarId'
--   let ht ← getLocalDecl n.mvarId!
--   exact (← mkEqMpr pf ht.toExpr)
...
not used anywhere, no need to re-implement
-/
