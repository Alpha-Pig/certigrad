...
lemma to_dist_congr {fshapes : List S} (k₁ k₂ : env → dvec T fshapes) : ∀ (inputs : env) (nodes : List Node),
...
noncomputable def is_gintegrable {shapes : List S} (k : env → dvec T shapes) {shape : S} : ∀ (m : env) (nodes : List Node) (f : dvec T shapes → T shape), Prop
| _ [] f => true
...
noncomputable def is_gdifferentiable (k : env → dvec T [[]]) : ∀ (tgt : Reference) (m : env) (nodes : List Node) (f : dvec T [[]] → ℝ), Prop
| tgt _ [] f := true
...
lemma is_gintegrable_k_congr {fshapes : List S} {fshape : S} (k₁ k₂ : env → dvec T fshapes) : ∀ (inputs : env) (nodes : List Node) (f : dvec T fshapes → T fshape),
  uniq_ids nodes inputs →
  (∀ (m : env), (∀ (ref : Reference), env.has_key ref inputs → env.get ref m = env.get ref inputs) → k₁ m = k₂ m) →
...
noncomputable def is_nabla_gintegrable (k : env → dvec T [[]]) : ∀ (tgt : Reference) (m : env) (nodes : List Node) (f : dvec T [[]] → ℝ), Prop
| tgt m [] f => true
