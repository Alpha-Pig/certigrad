...
@[grad_simp] lemma grad_chain_rule : ∀ {shape₁ shape₂ : S} (f : T shape₁ → T shape₂) (g : T shape₂ → TReal) (θ : T shape₁),
  ∇ (λ (θ₀ : T shape₁) => g (f θ₀)) θ = tmulT (D f θ) (∇ g (f θ)) := by exact grad_tmulT


@[grad_simp] axiom   grad_integral : ∀ {ishape tshape : S} (f : T ishape → T tshape → TReal) (θ : T tshape),
...
@[grad_simp] lemma grad_congr {shape : S} {f g : T shape → TReal} {x : T shape} (H : ∀ x, f x = g x) : ∇ f x = ∇ g x := by
  rw [funext H]
...
@[grad_simp] axiom   grad_sub₁ {shape : S} (k : T shape → TReal) (x₁ x₂ : T shape) :
  ∇ (λ x₁ => k (x₁ - x₂)) x₁ = ∇ k (x₁ - x₂)

@[grad_simp] axiom   grad_sub₂ {shape : S} (k : T shape → TReal) (x₁ x₂ : T shape) :
  ∇ (λ x₂ => k (x₁ - x₂)) x₂ = - ∇ k (x₁ - x₂)

@[grad_simp] axiom   grad_mul₁ {shape : S} (k : T shape → TReal) (x₁ x₂ : T shape) :
  ∇ (λ x₁ => k (x₁ * x₂)) x₁ = ∇ k (x₁ * x₂) * x₂
...
-- Tensors
@[grad_simp] axiom   grad_sum (k : TReal → TReal) (shape : S) (x : T shape) :
  ∇ (λ x => k (sum x)) x = ∇ k (sum x) • 1

@[grad_simp] axiom   grad_dot₁ {shape : S} (x₁ x₂ : T shape) : ∇ (λ x₁ => dot x₁ x₂) x₁ = x₂
@[grad_simp] axiom   grad_dot₂ {shape : S} (x₁ x₂ : T shape) : ∇ (λ x₂ => dot x₁ x₂) x₂ = x₁

@[grad_simp] axiom   grad_gemm₁ {m p : ℕ} (k : T [m, p] → TReal) (n : ℕ) (M : T [m, n]) (N : T [n, p]) :
...
-- Congruences
@[grad_simp] axiom   grad_congr_pos {shape : S} (f g : T shape → TReal) (θ : T shape) :
  θ > 0 → (∀ (θ₀ : T shape), θ₀ > 0 → f θ₀ = g θ₀) → ∇ f θ = ∇ g θ

-- Compound
@[grad_simp] lemma grad_softplus {shape : S} (k : T shape → TReal) (θ : T shape) :
...
@[simp] lemma grad_sigmoid {shape : S} (k : T shape → TReal) (θ : T shape) :
...
@[grad_simp] lemma grad_add_fs {ishape : S} (θ : T ishape) (f₁ f₂ : T ishape → TReal) :
...
@[simp] lemma trivial_mul_one  (α : TReal) (shape : S): (const α shape * 1) = const α shape := by simp

@[simp] lemma grad_scale_f {ishape : S} (θ : T ishape) (α : TReal) (f : T ishape → TReal) :
  ∇ (λ θ₀ => α • f θ₀) θ = α • ∇ (λ θ₀ => f θ₀) θ := by
  rw [grad_chain_rule f (λ θ => α • θ) θ]
  rw [grad_scale (λ θ => θ)]
  rw [grad_id]
  rw [TReal_one] -- we introduce a new @[grad_simp] axiom   here
  rw [tmulT_scalar]
  rw [D_scalar]
...
set_option trace.Meta.Tactic.simp true -- Lean 4
example {ishape oshape : S} (f : T ishape → T oshape) (k : T oshape → TReal) (θ : T ishape) :
  ∇ (λ θ₀ => k (f θ₀)) θ = tmulT (D (λ θ₀ => f θ₀) θ) (∇ k (f θ)) := by
  simp [grad_simp]


elab "simplifyGrad": tactic =>
  withMainContext do
    logInfo m!"[CertiGrad.T.simplifyGrad] Attempting to simplify gradient expression..."
    try
      -- 构造 simp 调用。用户可以通过 `simplifyGrad (config := {...})` 传入自定义配置
      -- 这里我们使用一个合理的默认配置，并允许用户覆盖。
      -- `only [*] with ...` 确保主要使用您指定的规则集和局部假设。
      -- 如果您希望全局的 @[simp] 规则也参与主要化简，可以去掉 `only`。
      evalTactic (← `(tactic|
        simp (config := {
                zeta := false, beta := false, eta := false, proj := false, -- 来自您的 Lean 3 配置
                arith := true, -- 根据需要开启
                maxSteps := 200000, -- 增加步数限制
                maxDischargeDepth := 10, -- 增加前提解决深度
                failIfUnchanged := false
              })
        only [*, @grad_simp, @cdiff_simp, @precond_simp]
      ))
      logInfo m!"[CertiGrad.T.simplifyGrad] Finished."
    catch e =>
      logError m!"[CertiGrad.T.simplifyGrad] Failed: {e.toMessageData}"
      throw e
